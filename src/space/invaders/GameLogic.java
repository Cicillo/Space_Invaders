...
public class GameLogic {
	...
	private Consumer<String> statusMessage;
	...
	public void initialize(GamePane pane, Consumer<String> statusMessage) {
	...
	public ObservableIntegerValue getLevel() {
		return currentLevel;
	}
	...
	private Enemy makeEnemy(IntegerCoordinates coords) {
		char c = levels.get(currentLevel.get())[coords.getY()].charAt(coords.getX());
		switch (c) {
			case 'N':
	...
	public void removePlayerLife(int qty) {
		...
		if ((remainingLives -= qty) <= 0) {
			// Game over
			gameState = -1;
			statusMessage.accept("You have lost!");
			return;
		}
	...
	private void winGame() {
		...
		if (++level >= levels.size()) {
			// Finished all levels: win the game.
			gameState = 1;
			statusMessage.accept("You have won!");
			return;
		}
